...
in.file <- "/Users/rmvpaeme/test/WiseX_JVW/scripts/SRR9094547_trimmed_plot_tmp.json"
...
out.dir <- "/Users/rmvpaeme/test/WiseX_JVW/scripts/SRR9094547"
...
ratio_auto <- ratio[1:chr.ends[length(chrs) - 1]]
weights <- weights[1:chr.ends[length(chrs) + 1]]

ratio_gono <- ratio[chr.ends[length(chrs) - 1 ]:chr.ends[length(chrs) + 1]] - 1

ratio <- c(ratio_auto, ratio_gono)
...
for (ab in input$results_c){
  ...
  if ((chr == 23 | chr == 24) & gender == "M"){
    ploidy = 1
    z = as.double(info[4]) - 1
    height = as.double(info[5]) - 1
  }
